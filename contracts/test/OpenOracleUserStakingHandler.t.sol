...
contract OpenOracleUserStakingTest is Test {
    ...
    OpenOracleUserStakingHandler public stakeContract;
    ...
    function setUp() public {
        ...
        stakeContract = OpenOracleUserStakingHandler(address(proxy));
    }
    ...
    function testStake() public {
        address user = address(this);
        stakeContract.addToken(address(token), 1000000);
        stakeContract.afterStake(user, address(token), 10**18);
    ...
    function testFailStakeZero() public {
        address user = address(this);
        stakeContract.addToken(address(token), 1000000);
        stakeContract.afterStake(user, address(token), 0);
    }

    function testFailStakeNotExist() public {
        address user = address(this);
        stakeContract.addToken(address(token), 1000000);
        stakeContract.afterStake(user, address(0), 0);
    }
    ...
    function testLessStake() public {
        address user = address(this);
        stakeContract.addToken(address(token), 1000000);
        stakeContract.afterStake(user, address(token), 10**17);
    ...
    function testUnStake() public {
        testStake();
        address user = address(this);
        stakeContract.afterUnstake(user, address(token), 10**18);
    ...
    function testManyUnStake() public {
        testManyStake();
        address user = address(this);
        stakeContract.afterUnstake(user, address(token), 11*10**17);
        assertEq(stakeContract.tokenStake(user, address(token)),9*10**17);
        assertEq(stakeContract.totalStake(user),9*10**5);
        stakeContract.afterUnstake(user, address(token), 9*10**17);
        assertEq(stakeContract.tokenStake(user, address(token)),0);
        assertEq(stakeContract.totalStake(user),0);
    }
    ...
    function testUnStakeWithLess() public {
        testLessStake();
        address user = address(this);
        stakeContract.afterUnstake(user, address(token), 10**17);
        assertEq(stakeContract.tokenStake(user, address(token)),0);
        assertEq(stakeContract.totalStake(user),0);
    }
    ...
    function testFailUnStakeZero() public {
        testStake();
        address user = address(this);
        stakeContract.afterUnstake(user, address(token), 0);
    }

    function testFailUnStakeNotExist() public {
        testStake();
        address user = address(this);
        stakeContract.afterUnstake(user, address(0), 0);
    }

    function testFailUnStakeNotEnough() public {
        testStake();
        address user = address(this);
        stakeContract.afterUnstake(user, address(token), 2*10**18);
    }
